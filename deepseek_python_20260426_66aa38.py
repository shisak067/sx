...
from config import (
    MONGO_URI, MONGO_DB_NAME, OWNER_ID, 
    DEFAULT_ADMIN_PASSWORD, DEFAULT_TEMP_ADMIN_TIMEOUT,
    CACHE_TTL
)
...
class CacheManager:
    def __init__(self):
        self.user_cache = TTLCache(maxsize=1000, ttl=CACHE_TTL)
...
class Database:
    ...
    def initialize_data(self):
        if self.apk_messages.count_documents({}) == 0:
            self.apk_messages.insert_one({
                "message_id": 3, "channel_id": -1003960878285, 
                "name": "Main APK", "created_at": datetime.now()
            })
    ...
    def init_admin_password(self):
        ...
        if not setting:
            self.bot_settings.insert_one({
                "key": "admin_password", "value": DEFAULT_ADMIN_PASSWORD,
                "updated_at": datetime.now(), "updated_by": OWNER_ID
            })
    ...
    def set_admin_password(self, new_password: str, updated_by: int):
        self.bot_settings.update_one(
            {"key": "admin_password"},
            {"$set": {"value": new_password, "updated_at": datetime.now(), "updated_by": updated_by}},
            upsert=True
        )
    ...
    def generate_login_key(self, created_by: int, max_uses: int = 1, duration_hours: int = 24, note: str = "") -> str:
        ...
        self.login_keys.insert_one({
            "key": key, "created_by": created_by, "created_at": datetime.now(),
            "expires_at": expires_at, "max_uses": max_uses, "used_count": 0,
            "is_active": True, "note": note, "users_used": []
        })
        return key
    ...
    def use_login_key(self, key: str, user_id: int) -> bool:
        result = self.login_keys.update_one(
            {"key": key, "is_active": True, "expires_at": {"$gt": datetime.now()},
             "$expr": {"$lt": ["$used_count", "$max_uses"]}},
            {"$inc": {"used_count": 1}, "$push": {"users_used": {"user_id": user_id, "used_at": datetime.now()}}}
        )
    ...
    def create_temp_admin_session(self, user_id: int, duration_seconds: int = DEFAULT_TEMP_ADMIN_TIMEOUT, used_key: str = None):
        ...
        self.admin_sessions.insert_one({
            "user_id": user_id, "created_at": datetime.now(), "expires_at": expires_at,
            "is_active": True, "used_key": used_key, "login_count": 1
        })
        
        self.temp_admins.update_one(
            {"user_id": user_id},
            {"$set": {"last_login": datetime.now(), "expires_at": expires_at, "used_key": used_key},
             "$setOnInsert": {"first_login": datetime.now(), "login_count": 0},
             "$inc": {"login_count": 1}},
            upsert=True
        )
    ...
    def is_temp_admin(self, user_id: int):
        session = self.admin_sessions.find_one({
            "user_id": user_id, "expires_at": {"$gt": datetime.now()}, "is_active": True
        })
    ...
    # FIXED: terminate_temp_admin with better handling
    def terminate_temp_admin(self, user_id: int, terminated_by: int) -> bool:
        session = self.admin_sessions.find_one({"user_id": user_id})
        if session:
            self.admin_sessions.delete_many({"user_id": user_id})
            self.temp_admins.update_one(
                {"user_id": user_id},
                {"$set": {"terminated_by": terminated_by, "terminated_at": datetime.now(), "is_terminated": True}}
            )
            cache.set_admin_cache(user_id, None)
            return True
        return False
    
    # FIXED: extend_temp_admin_session
    def extend_temp_admin_session(self, user_id: int, additional_seconds: int = DEFAULT_TEMP_ADMIN_TIMEOUT):
        session = self.admin_sessions.find_one({"user_id": user_id})
        if not session:
            return None
        
        new_expiry = datetime.now() + timedelta(seconds=additional_seconds)
        self.admin_sessions.update_one(
            {"user_id": user_id},
    ...
    def get_temp_admin_stats(self):
        ...
        return {
            ...
            "active_sessions": active_temp_sessions,
            "total_logins": stats[0]["total_logins"] if stats else 0,
            "avg_logins": round(stats[0]["avg_logins"], 2) if stats else 0
        }
    ...
    def ban_user(self, user_id: int, banned_by: int, reason: str = None):
        try:
            ...
            self.banned_users.update_one(
                {"user_id": user_id},
                {"$set": {"banned_by": banned_by, "banned_at": datetime.now(),
    ...
    def add_user(self, user_id: int, username: str = None, first_name: str = None):
        ...
        self.users.update_one(
            ...
            {"$set": {"username": username, "first_name": first_name, "last_active": datetime.now()},
             "$setOnInsert": {"created_at": datetime.now(), "age_verified": False, "gender": None, "welcome_sent": False}},
            upsert=True
        )
        ...
        try:
            self.user_records.update_one(
                ...
                {"$set": {"username": username, "first_name": first_name, "last_name": last_name,
                         "full_name": f"{first_name or ''} {last_name or ''}".strip(), "last_updated": datetime.now()},
                 "$setOnInsert": {"created_at": datetime.now(), "total_logins": 0, "total_commands": 0,
                                 "total_apk_downloads": 0, "total_demo_views": 0, "last_active": datetime.now(), "status": "active"}},
                upsert=True
            )
            return True
        except Exception as e:
    ...
    def add_user_login(self, user_id: int, login_type: str = "start"):
        try:
            self.user_logins.insert_one({
                "user_id": user_id, "login_type": login_type, "login_time": datetime.now(),
                "ip": None, "user_agent": "Telegram Bot"
            })
            self.user_records.update_one(
                {"user_id": user_id},
                {"$inc": {"total_logins": 1, "total_commands": 1}, "$set": {"last_active": datetime.now()}}
            )
            return True
        except Exception as e:
    ...
    def add_user_action(self, user_id: int, action_type: str, action_details: str = None):
        try:
            ...
            if update_fields:
                self.user_records.update_one(
                    {"user_id": user_id},
                    {**update_fields, "$set": {"last_active": datetime.now()}}
                )
            
            if action_details:
                self.user_actions.insert_one({
                    "user_id": user_id, "action_type": action_type,
                    "action_details": action_details, "timestamp": datetime.now()
                })
            return True
        except Exception as e:
    ...
    def search_users(self, search_term: str):
        try:
            ...
            return list(self.user_records.find({
    ...
    def get_user_stats_summary(self):
        ...
        return {"total": total_users, "active_today": active_today,
                "active_week": active_week, "active_month": active_month, "top_users": top_users}
    
    # ========== VIDEO MANAGEMENT ==========
    def add_welcome_video(self, video_file_id: str, caption: str = None):
        return self.welcome_videos.insert_one({
            "video_file_id": video_file_id, "caption": caption, "created_at": datetime.now()
        }).inserted_id
    ...
    def add_demo_video(self, video_file_id: str, caption: str = None):
        return self.demo_videos.insert_one({
            "video_file_id": video_file_id, "caption": caption, "created_at": datetime.now()
        }).inserted_id
    ...
    def has_howto_content(self):
        text_exists = self.howto_texts.count_documents({}) > 0
        video_exists = self.howto_videos.count_documents({}) > 0
        return text_exists or video_exists
    
    # ========== APK MANAGEMENT ==========
    def add_apk_message(self, message_id: int, channel_id: int, name: str = "APK File"):
        try:
            return self.apk_messages.insert_one({
                "message_id": message_id, "channel_id": channel_id, "name": name, "created_at": datetime.now()
            }).inserted_id
        except:
            return None
